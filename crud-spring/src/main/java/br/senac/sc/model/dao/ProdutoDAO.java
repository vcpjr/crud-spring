...
public class ProdutoDAO {
	...
	public int inserir(Produto p) {
		...
		PreparedStatement prepStmt = Banco.getPreparedStatementWithPk(conexao, sql);
	...
	private Produto construirProdutoDoResultSet(ResultSet result) {
		...
		try {
			...
			p.setDataCadastro(result.getDate("DATA_CADASTRO").toLocalDate());
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return p;
	...
	//1 - Incluir um seletor como parâmetro do método (para trazer
	//tudo que foi preenchido pelo usuário na tela de filtro)
